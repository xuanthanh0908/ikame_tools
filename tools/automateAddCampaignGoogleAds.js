      ...
      .then(async (path) => {
        ...
        try {
          ...
          const app_promote_path =
            "dynamic-component[data-value=APP_DOWNLOADS]";
          await driver
            .wait(
            ...
            .then(async () => {
              const findEl = await driver.findElement(By.css(app_promote_path));
              await driver.executeScript("arguments[0].click()", findEl);
              const select_camp_type_css = "channel-selection-card-v2";
              await driver
                .wait(
                  until.elementLocated({
                    css: select_camp_type_css,
                  }),
                  maxTime
                )
                .then(async () => {
                  const select_camp_type = await driver.findElement(
                    By.css(select_camp_type_css)
                  );
                  ...
                  const some_path_loading =
                    "//div[normalize-space()='Android']";
                  ...
                  await driver.wait(conditions_01, maxTime).then(async () => {
                    ...
                    await driver.wait(condition_04, maxTime).then(async () => {
                      await driver
                        ...
                        .then(async () => {
                          const select_app_path = ".app-info";
      ...
      if (DATA.location_to_target === "Enter another location") {
        await driver.wait(conditions_01, max_time).then(async () => {
          ...
          for (const [index, loca] of DATA.locations.entries()) {
            console.log("============SELECT LOCATION===========", loca);
            const input_path_01 =
        ...
        await driver
          ...
          .then(async () => {
            ...
            await driver
              ...
              .click()
              .then(async () => {
                // enable language ---- SELECT LANGUAGE ----
                ...
                if (languageArr.length > 0 && languageArr[0] !== "All") {
                  ...
                  await driver
                    .executeScript("arguments[0].scrollIntoView(true)", findEl)
                    .then(async () => {
                      await driver.executeScript(
                        "arguments[0].click()",
                        findEl
...
const nextAction = async (DATA, driver, userId, id) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const next_button_css =
        "//dynamic-component[@class='content-element _ngcontent-awn-CM_EDITING-39']//div[@class='_ngcontent-awn-CM_EDITING-42']//material-button[@role='button']";
      const next = await driver.findElement(By.xpath(next_button_css));
      await driver
        .executeScript("arguments[0].click()", next)
        .then(async () => {
